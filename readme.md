...
## home screen:
  [create game -> creates new room with name made of 
      rnd adj & rnd noun or rnd adv & rnd verb]
...
## issues:
  -⁉️what if player enters crap question or “asdf”?
    (low quality question)
    -🔧ameliorate w min char q’s
    -🔧ameliorate w reassigning p’s q (if their q sucks)
  -⁉️players may leave by accident, join with a different browser
    (player reconnection) - solution - if any p gets a bad q, mod can reassign random q
    -what should be done in this case? is this only a problem if they submitted a question?
    -🔧ameliorate w mod features (kick)
  -⁉️💯🤷‍♂️what if two players join at the same time
    (concurrent join) - solution - just save exact time joined, and it’ll be saved as their player ID in local storage
